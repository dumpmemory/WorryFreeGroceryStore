...
class KeywordProcessor(object):
	...
	def __setitem__(self, keyword, clean_name=None):
		...
		if keyword and clean_name:
			...
			#-----------had made a change----------
			#because some keywords may have multi-label, we use '|' to split them
					...
					if char in current_dict:
						...
						while idy < sentence_len:
							...
							if self._keyword in current_dict_continued:
								...
								is_longer_seq_found = True
						if is_longer_seq_found:
							###change this line by loulianzhang in 20191009
							#idx = sequence_end_pos
							idx = sequence_end_pos-1
							current_word = current_word_continued
					current_dict = self.keyword_trie_dict
					if longest_sequence_found:
						###change this line by loulianzhang 20191009
						#new_sentence += longest_sequence_found + current_white_space
						new_sentence += longest_sequence_found
						current_word = ''
						current_white_space = ''
					else:
...
if __name__ == "__main__":
	kw = KeywordProcessor()
	kw.add_keyword_from_file("D:/data/nlu/life/reservation/reject.txt")
	print(kw.extract_keywords("定位票"))
